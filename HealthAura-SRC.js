/* global libTokenMarkers createObj TokenMod spawnFxWithDefinition getObj state playerIsGM sendChat _ findObjs log on*/
/*
...
------------------------*/
        ...
        //CHECK IF TRIGGERED------------
            ...
            if(obj.get('imgsrc').match(/files.d20.io\/images\/4277467\/iQYjFOsYC5JsuOPUCI9RGA\//)) return;
            ...
            //**CHECK BARS------------//
                ...
                if(isNaN(maxValue) || isNaN(curValue) || isNaN(prevValue)) return;
                        if (1 === 2){    
                            const activeStatuses = obj.get("statusmarkers").split(',');
                            log(activeStatuses);
                            for (let Tokmarker of activeStatuses){             
                                    obj.set('status_'+Tokmarker, false);
                                    //log('status_'+Tokmarker);
                            };
                            return;
                        };
            //CALC PERCENTAGE------------
            ...
            //Status Icons By Health
                var HealthStatusMarkerA = false;
                var HealthStatusMarkerB = false;
                var HealthStatusMarkerC = false;
                if (percReal <= state.HealthColors.Precent_StatusC && percReal > 0){
                    HealthStatusMarkerA = false;
                    HealthStatusMarkerB = false;
                    HealthStatusMarkerC = true;
                }else if (percReal > state.HealthColors.Precent_StatusC && percReal <= state.HealthColors.Precent_StatusB){
                    HealthStatusMarkerA = false;
                    HealthStatusMarkerB = true;
                    HealthStatusMarkerC = false;
                }else if (percReal > state.HealthColors.Precent_StatusB && percReal <= state.HealthColors.Precent_StatusA){
                    HealthStatusMarkerA = true;
                    HealthStatusMarkerB = false;
                    HealthStatusMarkerC = false;
                }else{
                    HealthStatusMarkerA = false;
...
//log(obj.get('statusmarkers')+ " - " + HealthStatusMarkerA + " - " + state.HealthColors.Icon_StatusA + " - " + HealthStatusMarkerB + " - " + HealthStatusMarkerC );
            //DEFINE VARIABLES---
                var pColor = '#ffffff';
                var GM = '',PC = '';
                var IsTypeOn, PercentOn, ShowDead, UseAura, ShowUnconcious;
        ...
        //CHECK DISABLED AURA/TINT ATTRIB------------
            ...
            //SET HEALTH COLOR----------
                if(IsTypeOn && UseAura !== "NO") {
                    ...
                    if (!state.HealthColors.auraIcon){
                        libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA).removeFrom(obj);
                        libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB).removeFrom(obj);
                        libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC).removeFrom(obj);
                    }else{
                        if (HealthStatusMarkerA){
                            //log('Status A is True - '+state.HealthColors.Icon_StatusA);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA).applyTo(obj);
                        }else{
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA).removeFrom(obj); 
                        };
                        if (HealthStatusMarkerB){
                            //log('Status B is True - '+state.HealthColors.Icon_StatusB);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB).applyTo(obj);
                        }else{
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB).removeFrom(obj); 
                        };
                        if (HealthStatusMarkerC){
                            //log('Status C is True - '+state.HealthColors.Icon_StatusC);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC).applyTo(obj);
                        }else{
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC).removeFrom(obj); 
                        };
                    }                    
                    if (state.HealthColors.Icon_StatusA !== state.HealthColors.Icon_StatusA_Org) libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA_Org).removeFrom(obj);
                    if (state.HealthColors.Icon_StatusB !== state.HealthColors.Icon_StatusB_Org) libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB_Org).removeFrom(obj);
                    if (state.HealthColors.Icon_StatusC !== state.HealthColors.Icon_StatusC_Org) libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC_Org).removeFrom(obj);
                    if (state.HealthColors.Icon_DeathMarker !== state.HealthColors.Icon_DeathMarker_Org) libTokenMarkers.getStatus(state.HealthColors.Icon_DeathMarker_Org).removeFrom(obj);
                    if (state.HealthColors.Icon_Unconcious !== state.HealthColors.Icon_Unconcious_Org) libTokenMarkers.getStatus(state.HealthColors.Icon_Unconcious_Org).removeFrom(obj);

            //SHOW DEAD----------
                    if(ShowDead === true) {
                        if(curValue > 0) libTokenMarkers.getStatus(state.HealthColors.Icon_DeathMarker).removeFrom(obj);
                        else if(curValue < 1) {
                            var DeadSounds = state.HealthColors.auraDeadFX;
                            if(DeadSounds !== "None" && curValue != prevValue) PlayDeath(DeadSounds);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_DeathMarker).applyTo(obj);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA).removeFrom(obj);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB).removeFrom(obj);
                            libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC).removeFrom(obj);
                            SetAuraNone(obj);
            ...
            if(msg.type == "api" && command.indexOf("!AURA") !== -1) {
                ...
                if(!playerIsGM(msg.playerid)) {
                    ...
                    case "TINT":
                        ...
                        break;
                    case "AURA":
                        state.HealthColors.auraAura = !state.HealthColors.auraAura;
                        break;
                    case "ICON":
                        state.HealthColors.auraIcon = !state.HealthColors.auraIcon;
                        break;
                    case "STATUSDEADICON":
                        IconListing("STATUSDEADICON", state.HealthColors.Icon_DeathMarker);
                        return;
                        break;
                    case "STATUSDEADICONAPPLY":
                        state.HealthColors.Icon_DeathMarker_Org = state.HealthColors.Icon_DeathMarker;
                        state.HealthColors.Icon_DeathMarker = decodeURI(msgFormula[2]);
                        break;
                    case "STATUSUNCONCIOUSICON":
                        IconListing("STATUSUNCONCIOUSICON", state.HealthColors.Icon_Unconcious);
                        return;
                        break;
                    case "STATUSUNCONCIOUSICONAPPLY":
                        state.HealthColors.Icon_Unconcious_Org = state.HealthColors.Icon_Unconcious;
                        state.HealthColors.Icon_Unconcious = msgFormula[2];
                        break;
                    case "STATUSAICON":
                        IconListing("STATUSAICON", state.HealthColors.Icon_StatusA);
                        return;
                        break;         
                    case "STATUSAICONAPPLY":
                        state.HealthColors.Icon_StatusA_Org = state.HealthColors.Icon_StatusA;
                        state.HealthColors.Icon_StatusA = msgFormula[2];
                        break;                             
                    case "STATUSBICON":
                        IconListing("STATUSBICON", state.HealthColors.Icon_StatusB);
                        return;
                        break;                            
                    case "STATUSBICONAPPLY":
                        state.HealthColors.Icon_StatusB_Org = state.HealthColors.Icon_StatusB;
                        state.HealthColors.Icon_StatusB = msgFormula[2];
                        break;                              
                    case "STATUSCICON":
                        IconListing("STATUSCICON", state.HealthColors.Icon_StatusC);
                        return;
                        break;                        
                    case "STATUSCICONAPPLY":
                        state.HealthColors.Icon_StatusC_Org = state.HealthColors.Icon_StatusC;
                        state.HealthColors.Icon_StatusC = msgFormula[2];
                        break;
                    case "PERC":
...
"FUNCTIONS"
------------------------*/
    // Show Status Icon List
        IconListing = function (iconRepresents,iconCurrent){
            const tokenMarkers = JSON.parse(Campaign().get("token_markers"));
            var tokenMarkersnotImg = "dead,red,blue,green,brown,purple,pink,yellow";
            ...
            var selectOpeningHTML = '<div style="border-radius: 8px 8px 8px 8px; padding: 5px; font-size: 9pt; text-shadow: ' + tshadow + '; box-shadow: 3px 3px 1px #707070; ' + img + ' color:#FFF; border:2px solid black; text-align:right; vertical-align:middle;">' + '<big>HealthStatus Version: ' + version + '</big><br>';
            var selectClosingHTML = '</div>';
            var lnkStyle = "style='background-color:#fff; padding: 0px;'"
            var lnkStyleSelected = "style='background-color:#fff000; padding: 0px; border:0px;'"
            var TokenMarkerArray = '';
            var strArray = JSON.stringify(tokenMarkers);
            const openButton =              "<div style='clear:left; display:inline-block; width: 25px; height: 25px; text-align:left; background-color:#fff; color: #eee; margin: 2px; padding: 4px 4px 4px 4px; overflow: hidden'>";
            const openButtonUsed =     "<div style='clear:left; display:inline-block; width: 25px; height: 25px; text-align:left; background-color:#444400; color: #eee; margin: 2px;padding: 4px 4px 4px 4px; overflow: hidden'>";
            const openButtonHighlight =     "<div style='clear:left; display:inline-block; width: 25px; height: 25px; text-align:left; background-color:#fff000; color: #eee; margin: 2px;padding: 4px 4px 4px 4px; overflow: hidden'>";
            const closeButton = "</div>";


            for (let MarkersnotImg of tokenMarkersnotImg.split(',')){    
                if (MarkersnotImg === iconCurrent){
                    TokenMarkerArray += openButtonHighlight + '<a '+lnkStyleSelected+' title="'+MarkersnotImg+'"  href="!aura menu">' +libTokenMarkers.getStatus(MarkersnotImg).getHTML(2,"margin: 0px;") + "</a>" + closeButton;
                }else if (MarkersnotImg === state.HealthColors.Icon_Unconcious || MarkersnotImg === state.HealthColors.Icon_DeathMarker || MarkersnotImg === state.HealthColors.Icon_StatusA || MarkersnotImg === state.HealthColors.Icon_StatusB || MarkersnotImg === state.HealthColors.Icon_StatusC) {
                    TokenMarkerArray += openButtonUsed + libTokenMarkers.getStatus(MarkersnotImg).getHTML(2,"margin: 0px;") + closeButton;
                }else{
                    TokenMarkerArray += openButton + '<a '+lnkStyle+' title="'+MarkersnotImg+'" href="!aura '+iconRepresents+'Apply '+encodeURIComponent(MarkersnotImg)+'">'  + libTokenMarkers.getStatus(MarkersnotImg).getHTML(2,"margin: 0px;") + "</a>" + closeButton;
                };
            };

            _.each(tokenMarkers, Tokmarker => {
                if (Tokmarker.name === iconCurrent){
                    TokenMarkerArray += openButtonHighlight + '<a '+lnkStyleSelected+' title="'+Tokmarker.name+'" href="!aura menu">'  + libTokenMarkers.getStatus(Tokmarker.name).getHTML(2,"margin: 0px;") + "</a>" +closeButton;
                }else if (Tokmarker.name === state.HealthColors.Icon_Unconcious || Tokmarker.name === state.HealthColors.Icon_DeathMarker || Tokmarker.name === state.HealthColors.Icon_StatusA || Tokmarker.name === state.HealthColors.Icon_StatusB || Tokmarker.name === state.HealthColors.Icon_StatusC) {
                    TokenMarkerArray += openButtonUsed + libTokenMarkers.getStatus(Tokmarker.name).getHTML(2,"margin: 0px;") + closeButton;
                }else{
                    TokenMarkerArray += openButton + '<a '+lnkStyle+' title="'+Tokmarker.name+'" href="!aura '+iconRepresents+'Apply '+encodeURIComponent(Tokmarker.name)+'">'  + libTokenMarkers.getStatus(Tokmarker.name).getHTML(2,"margin: 0px;") + "</a>" +closeButton;
                };
                
            });        

            
            sendChat('HealthColors', "/w GM <b><br>" + selectOpeningHTML + HR + TokenMarkerArray + selectClosingHTML );            
    ...
    //SET TOKEN COLORS------------
        TokenSet = function (obj, sizeSet, markerColor, pColor, update) {
            ...
            if(state.HealthColors.auraAura === true) {
                obj.set({
    ...
    //REMOVE ALL------------
        SetAuraNone = function (obj) {
            ...
            if(state.HealthColors.auraAura === true) obj.set({'aura1_color': "transparent",'aura2_color': "transparent",});
        },
    ...
    //HELP MENU------------
        aurahelp = function (OPTION) {
            ...
            var IconStatus = state.HealthColors.auraIcon;
            var Icon_Menu_Items = "";
            var tokenMarkers = JSON.parse(Campaign().get("token_markers"));
            var getTokenMarkersArray = markers => {
                let TokenMarkerArray = '|dead|red|blue|green|brown|purple|pink|yellow|';
                _.each(markers, marker => {
                    //TokenMarkerArray += `${marker.name},${marker.id}::${marker.name}|`;::${marker.id}
                    TokenMarkerArray += `${marker.name}|`;
                });
                return TokenMarkerArray;
            };
            var TokenMarkerArrayFull = getTokenMarkersArray(tokenMarkers);
            var StatusPrecentList = '95|90|85|80|75|70|65|60|55|50|45|40|35|30|25|20|15|10|5'
            
            
            if(IconStatus){
                Icon_Menu_Items = HR + 
                    'Status Dead: <a ' + style + '" href="!aura statusDeadIcon">' + libTokenMarkers.getStatus(state.HealthColors.Icon_DeathMarker).getHTML() + '</a><br>' + //--
                    //'Status Unconcious: <a ' + style + '" href="!aura statusUnconciousIcon">' + libTokenMarkers.getStatus(state.HealthColors.Icon_Unconcious).getHTML() + '</a><br>' + //--
                    'Status Hurt: <a ' + style + '" href="!aura statusAIcon">' + libTokenMarkers.getStatus(state.HealthColors.Icon_StatusA).getHTML() + '</a><br>' + //--
                    'Hurt at Precent: <a ' + style + '" href="!aura statusAPrecent ?{Precent|'+state.HealthColors.Precent_StatusA+'|'+StatusPrecentList+'}">' + state.HealthColors.Precent_StatusA + '%</a><br>' + //--
                    'Status Bloodied: <a ' + style + '" href="!aura statusBIcon">' + libTokenMarkers.getStatus(state.HealthColors.Icon_StatusB).getHTML() + '</a><br>' + //--
                    'Bloodied at Precent: <a ' + style + '" href="!aura statusBPrecent ?{Precent|'+state.HealthColors.Precent_StatusB+'|'+StatusPrecentList+'}">' + state.HealthColors.Precent_StatusB + '%</a><br>' + //--
                    'Status Rough: <a ' + style + '" href="!aura statusCIcon">' + libTokenMarkers.getStatus(state.HealthColors.Icon_StatusC).getHTML() + '</a><br>' + //--
                    'Rough at Precent: <a ' + style + '" href="!aura statusCPrecent ?{Precent|'+state.HealthColors.Precent_StatusC+'|'+StatusPrecentList+'}">' + state.HealthColors.Precent_StatusC + '%</a><br>';//-- --                
                ;
            }else{
                Icon_Menu_Items = "";
            }
            ...
            sendChat('HealthColors', "/w GM <b><br>" + '<div style="border-radius: 8px 8px 8px 8px; padding: 5px; font-size: 9pt; text-shadow: ' + tshadow + '; box-shadow: 3px 3px 1px #707070; ' + img + ' color:#FFF; border:2px solid black; text-align:right; vertical-align:middle;">' + '<big>HealthStatus Version: ' + version + '</big><br>' + //--
                HR + //--
                ...
                'Use Aura: <a ' + style + 'background-color:' + (state.HealthColors.auraAura !== true ? off : "") + ';" href="!aura aura">' + (state.HealthColors.auraAura !== true ? "No" : "Yes") + '</a><br>' + //--
                'Use Icons: <a ' + style + 'background-color:' + (state.HealthColors.auraIcon !== true ? off : "") + ';" href="!aura icon">' + (state.HealthColors.auraIcon !== true ? "No" : "Yes") + '</a><br>' + //--
    ...
    //CHECK INSTALL & SET STATE------------
        checkInstall = function () {
            ...
            if(_.isUndefined(state.HealthColors.auraAura)) state.HealthColors.auraAura = false; //use Aura instead?
            if(_.isUndefined(state.HealthColors.auraIcon)) state.HealthColors.auraIcon = false; //use Icon instead?
            ...
            //-----------------
            if(_.isUndefined(state.HealthColors.Icon_DeathMarker)) state.HealthColors.Icon_DeathMarker = "dead"; //Death Icon?
            if(_.isUndefined(state.HealthColors.Icon_Unconcious)) state.HealthColors.Icon_Unconcious = "skull"; //PC Unconcitious Icon?
            if(_.isUndefined(state.HealthColors.Icon_StatusA)) state.HealthColors.Icon_StatusA = "green"; //Status A Icon?
            if(_.isUndefined(state.HealthColors.Icon_StatusB)) state.HealthColors.Icon_StatusB = "yellow"; //Status B Icon?
            if(_.isUndefined(state.HealthColors.Icon_StatusC)) state.HealthColors.Icon_StatusC = "red"; //Status C Icon?
            if(_.isUndefined(state.HealthColors.Precent_StatusA)) state.HealthColors.Precent_StatusA = 90; //Status A Precentage max?
            if(_.isUndefined(state.HealthColors.Precent_StatusB)) state.HealthColors.Precent_StatusB = 60; //Status B Precentage max?
            if(_.isUndefined(state.HealthColors.Precent_StatusC)) state.HealthColors.Precent_StatusC = 30; //Status C Precentage max?
